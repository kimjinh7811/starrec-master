...
void c_top_k_index(float *ratings, int rating_len, int top_k, int *result)
{
    ...
    std::partial_sort_copy(index.begin(), index.end(), result, result+top_k, [& ratings](int &x1, int &x2)->bool{return ratings[x1]>ratings[x2];});
}

void c_top_k_array_index(float *scores_pt, int columns_num, int rows_num, int max_k, int *rankings_pt)
{
    // ThreadPool pool(10);
